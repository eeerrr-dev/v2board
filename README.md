...
4. 将保留数据写入全新专用的 PostgreSQL 18 cluster（执行前只有 `postgres` maintenance database）；
...
typed row，逐字段校验和转换，再通过 PostgreSQL driver 使用参数化批量写入；两种 SQL dialect、
类型、自增和约束的差异由 converter 显式处理。converter 从旧机通过临时 migration principal
写入新 PostgreSQL；新生产机不运行 MySQL。导入器不连接旧 Redis，也不联系 Stripe provider。
...
MySQL 的只读一致性快照、写入全新 target 并生成配置输出包；它不连接旧 Redis 或 Stripe，
也不会在新生产机运行 MySQL。
