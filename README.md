...
当前 lifecycle CLI 为 archive-first 旧版冷导入暴露以下命令：

- `v2board-lifecycle validate --manifest <path>`：严格校验 lifecycle JSON，不连接数据存储。
- `v2board-lifecycle inspect --manifest <path>`：只读 hash encrypted MySQL dump、age identity 和 release，
  校验文件与 archive 契约；不连接旧站或 live target。
...
- `v2board-lifecycle apply --manifest <path>`：production gate 解除后，从同一个不可变 dump 启动一次
  destructive cold import。

旧版 schema v5 不存在 `authorize` 或 `resume`。未激活 target 失败时必须清理 target 和隔离恢复库，
使用同一个 encrypted dump 与新 operation ID 从头重跑。`apply` 在 archive-first 集成门和最终安全审计
通过前继续失败关闭；当前检查统一输出 `apply_available=false`，因此不能获得迁移、写入或切流许可。
当前 CLI 只接受严格的
[旧版迁移 schema v5](docs/examples/legacy-migration.v5.example.json)；
[全新安装 v3](docs/examples/fresh-install.v3.example.json)与
[native 升级 v3](docs/examples/native-upgrade.v3.example.json)只保留为未来目标规范，当前 `validate`/`inspect`
都会失败关闭。旧版迁移中，旧节点、路由、历史流量
明细、旧请求/邮件日志和旧运行时文件都不进入新系统，节点在迁移后手工重建。

操作者先完全停止旧站，再生成 age 加密、SHA-256 绑定的 Oracle MySQL 8 dump。迁移器只从隔离恢复快照
转换，不接收旧 MySQL/Redis live URL、systemd unit 或 fence 凭据，也不读取旧 Redis。pending traffic、
queue/failed work、session、OTP、临时订阅链接、cache/lock 和 Horizon metadata 明确牺牲；MySQL 已落盘的
`v2_user.u/d/transfer_enable`、永久 `v2_user.token`、余额、套餐和其他保留业务值继续验证。Stripe 配置与
未完成 Stripe 订单不进入 target，provider-side object 完全不访问；非 Stripe payment 配置保留，已终结
Stripe 订单仅保留解除 provider binding 的业务历史。ClickHouse 从空 native event epoch 开始。

这是一次性离线迁移，不是长期共存方案：不实现 CDC、MySQL→PostgreSQL 复制、影子读、双写、分批放量
或旧库运行时回退。旧站停机、导出与最终退役由操作者在迁移器外负责；唯一永久 source archive 是经过
摘要和隔离恢复验证的 encrypted dump。新版启用前失败就清理未激活 target 后从该 dump 重跑；新版启用后
只允许 PostgreSQL/ClickHouse 恢复或 forward repair，不能重新接入旧 MySQL。
...
`/var/lib/v2board/api/config.json` 和 `/var/lib/v2board/worker/config.json`。最终 cold importer 必须按角色
分目录、owner/mode 校验、no-clobber 原子安装和回读验证；当前 apply 门禁关闭，不会执行这些写入。
...
active revision，不生成 API-only seed 文件。任何错值、孤儿 revision 或 state 漂移都阻断；legacy v5
在 activation 前崩溃时清理未激活 target 并从同一 dump 重跑，不续接 checkpoint。API/Worker 在 active
revision 存在并完成
...
一次性 `v2board-lifecycle` 是独立 cold-import tool，不进入长期 native release。当前只读实现读取
encrypted dump、age identity 和 native release；未来生产 importer 也只能读取 operation-owned 隔离 MySQL，
不能连接 live 旧站。迁移验收后由操作者删除该工具；这不依赖旧 source unit、旧凭据可达性或旧 Redis
receipt。普通
