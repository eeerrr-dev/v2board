...
- `v2board-lifecycle authorize ...`：重新执行并精确匹配已复查的稳定 `review_binding_sha256`，只有操作者
  输入完整 `operation_id` 后才写 owner-only authorization；authorization 另存当次完整动态 report digest，
  但它不 fence、不停服、不改 datastore。
...
[旧版迁移](docs/examples/legacy-migration.v4.example.json)和
[native 升级](docs/examples/native-upgrade.v3.example.json)；旧版迁移只有 schema v4 可以进入确认，v2 会在
...
旧 session 不转换，迁移后全量登出；旧版三张日统计表逐值保留在 PostgreSQL，不能展开成并不存在的
ClickHouse raw event。ClickHouse 从空的 native event epoch 开始，只接收迁移后经过验证的 PostgreSQL
outbox 投影，也不能用 ClickHouse 行数证明事务迁移成功。
