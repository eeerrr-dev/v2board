...
3. 在已停写的旧生产机上，把 dump 导入隔离的一次性 MySQL 8 engine；
4. 转换保留数据到全新专用的 PostgreSQL 18 cluster（执行前只有 `postgres` maintenance database）；
...
旧 MySQL 不被修改，staging 只用于处理 dump。默认在停写后的旧生产机运行，但必须使用独立
data directory/volume、端口、凭据和仅本机监听的第二个 MySQL instance；不能在原 MySQL instance
里创建 staging database，也不能挂载原 data directory。converter 从旧机通过临时 migration
principal 写入新 PostgreSQL。旧机容量不足时才改用一次性迁移 VM；新生产机不运行 staging 或
MySQL。导入器不连接旧 Redis，也不联系 Stripe provider。旧 MySQL source 表保留真实 `v2_*` 名；
...
- `v2board-lifecycle execute --manifest <path>`：从已加载 dump 的 staging MySQL 转换到全新
...
不完整的 staging、新 PostgreSQL/ClickHouse/Redis target 和输出目录，修正问题，再从 dump
...
平台维护，不由 V2Board unit 安装或启动。新生产机不安装或临时运行 MySQL；staging 与 converter
默认在停写后的旧生产机运行，旧机容量不足时使用一次性迁移 VM。
...
`validate`/`inspect` 检查清单与 dump，再由 `execute` 读取一次性 staging MySQL engine、写入全新
target 并生成配置输出包；整个过程不会连接旧 MySQL、旧 Redis 或 Stripe。该 engine 默认作为旧
生产机上的隔离临时 instance；旧机容量不足时才使用一次性迁移 VM，不在新生产机运行。
...
直接原子更新 `/opt/v2board/current`，依次启动 API、等待 `/readyz`，再启动 Worker。普通 schema job 只属于
未来真正存在新 migration 的 release，不是 MySQL converter 或首次启用步骤。

验收后由操作者删除旧机上的 lifecycle 工具、staging、manifest 和配置输出副本，撤销或轮换 PostgreSQL、ClickHouse、Redis 外部
