...
-- The importer is an adapter, not the native model. PostgreSQL uses native
-- booleans and normalized relations even when the legacy source/wire uses
-- integer flags or one-column-per-period shapes.
...
CREATE TABLE plan (
    ...
    created_at BIGINT NOT NULL,
    updated_at BIGINT NOT NULL,
    CONSTRAINT chk_plan_transfer_i32_range CHECK (
        transfer_enable BETWEEN 0 AND 2147483647
    ),
    CONSTRAINT chk_plan_device_limit_nonnegative CHECK (
        device_limit IS NULL OR device_limit >= 0
    ),
    CONSTRAINT chk_plan_speed_limit_nonnegative CHECK (
        speed_limit IS NULL OR speed_limit >= 0
    ),
    CONSTRAINT chk_plan_capacity_limit_nonnegative CHECK (
...
CREATE TYPE plan_price_period AS ENUM (
...
CREATE TABLE plan_price (
    ...
    period plan_price_period NOT NULL,
