...
CREATE INDEX idx_payment_archived ON payment(archived_at, id);
...
CREATE UNIQUE INDEX uniq_giftcard_code_canonical ON giftcard((lower(code)));
CREATE INDEX idx_giftcard_plan_id ON giftcard(plan_id);

CREATE TABLE giftcard_redemption (
    giftcard_id INTEGER NOT NULL REFERENCES giftcard(id) ON DELETE CASCADE,
    user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    created_at BIGINT NOT NULL,
    created_at_provenance TEXT NOT NULL DEFAULT 'native'
        CHECK (created_at_provenance IN ('native', 'legacy_unknown')),
    CONSTRAINT chk_giftcard_redemption_created_at_provenance CHECK (
...
CREATE INDEX idx_giftcard_redemption_user ON giftcard_redemption(user_id);

CREATE TABLE payment_reconciliation (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    payment_id INTEGER NOT NULL REFERENCES payment(id) ON DELETE RESTRICT,
...
CREATE INDEX idx_stat_server_record_at ON stat_server(record_at);
CREATE INDEX idx_stat_server_id ON stat_server(server_id);
...
CREATE INDEX idx_stat_user_record_at ON stat_user(record_at);
CREATE INDEX idx_stat_user_user_record_at ON stat_user(user_id, record_at DESC);
