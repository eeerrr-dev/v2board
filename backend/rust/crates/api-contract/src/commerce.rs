...
#[utoipa::path(
    get,
    operation_id = "adminPlansList",
    path = "/api/v1/{secure_path}/plans",
    params(("secure_path" = String, Path, description = "Live administrative path")),
    responses((status = 200, description = "All plans", body = [AdminPlanItem])),
    security(("bearer_auth" = [])),
    tag = "admin-plans"
)]
pub fn admin_plans_list_contract() {}

#[utoipa::path(
    post,
    operation_id = "adminPlanCreate",
    path = "/api/v1/{secure_path}/plans",
    params(("secure_path" = String, Path, description = "Live administrative path")),
    request_body = PlanCreate,
    responses((status = 201, description = "Created plan", body = CreatedId)),
    security(("bearer_auth" = [])),
    tag = "admin-plans"
)]
pub fn admin_plan_create_contract() {}

#[utoipa::path(
    patch,
    operation_id = "adminPlanPatch",
    path = "/api/v1/{secure_path}/plans/{id}",
    params(
        ("secure_path" = String, Path, description = "Live administrative path"),
        ("id" = i64, Path, description = "Plan identifier")
    ),
    request_body = PlanPatch,
    responses((status = 204, description = "Plan updated")),
    security(("bearer_auth" = [])),
    tag = "admin-plans"
)]
pub fn admin_plan_patch_contract() {}

#[utoipa::path(
    delete,
    operation_id = "adminPlanDelete",
    path = "/api/v1/{secure_path}/plans/{id}",
    params(
        ("secure_path" = String, Path, description = "Live administrative path"),
        ("id" = i64, Path, description = "Plan identifier")
    ),
    responses((status = 204, description = "Plan deleted")),
    security(("bearer_auth" = [])),
    tag = "admin-plans"
)]
pub fn admin_plan_delete_contract() {}

#[utoipa::path(
    post,
    operation_id = "adminPlansSort",
    path = "/api/v1/{secure_path}/plans/sort",
    params(("secure_path" = String, Path, description = "Live administrative path")),
    request_body = SortIdsRequest,
    responses((status = 204, description = "Plans reordered")),
    security(("bearer_auth" = [])),
    tag = "admin-plans"
)]
pub fn admin_plans_sort_contract() {}
