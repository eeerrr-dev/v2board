...
pub async fn project_or_verify_batch(
    ...
    match batch.event_name.as_str() {
        REPORTED_EVENT_NAME => {
            ...
            if !raw_present {
                let mut insert = client
                    .insert::<ReportedRow>("traffic_reported")
                    .await?
                    .with_setting(
            ...
            if !daily_present {
                let mut insert = client
                    .insert::<ReportedDailyRow>("traffic_reported_daily")
                    .await?
                    .with_setting(
                        "insert_deduplication_token",
        ...
        ACCOUNTED_EVENT_NAME => {
            ...
            if !raw_present {
                let mut insert = client
                    .insert::<AccountedRow>("traffic_accounted")
                    .await?
                    .with_setting(
                        "insert_deduplication_token",
            ...
            if !daily_present {
                let mut insert = client
                    .insert::<AccountedDailyRow>("traffic_accounted_daily")
                    .await?
                    .with_setting(
                        "insert_deduplication_token",
