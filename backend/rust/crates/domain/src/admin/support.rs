...
pub(super) fn payment_methods() -> Vec<&'static str> {
    SUPPORTED_PAYMENT_GATEWAYS.to_vec()
}

pub(super) fn payment_form(payment: &str) -> Value {
...
pub(super) fn form_field(form: &mut Map<String, Value>, key: &str, label: &str) {
    form.insert(
        key.to_string(),
        json!({
            "label": label,
