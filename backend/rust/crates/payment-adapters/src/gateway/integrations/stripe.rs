...
use super::{
    CheckoutResult, PaymentForCheckout, PaymentHttpClient, PaymentNotifyInput,
...
pub(in crate::gateway) const EXCHANGE_RATE_FRESH_TTL_SECS: i64 = 60 * 60;
pub(in crate::gateway) const EXCHANGE_RATE_STALE_TTL_SECS: i64 = 24 * 60 * 60;
...
pub(in crate::gateway) struct CachedExchangeRate {
    pub(in crate::gateway) rate: Decimal,
    pub(in crate::gateway) fetched_at: i64,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::gateway) enum ExchangeRateCacheDecision {
    Fresh(Decimal),
...
pub(in crate::gateway) fn add_stripe_settlement_metadata(
...
pub(in crate::gateway) fn stripe_source_charge_params(
    payment: &PaymentForCheckout,
    object: &Value,
...
pub(in crate::gateway) async fn stripe_credit_prepare(
    payment: &PaymentForCheckout,
    order: &PaymentOrder,
...
pub(in crate::gateway) fn reusable_stripe_credit_intent_matches(
    intent: &Value,
    trade_no: &str,
    payment_id: i32,
    user_id: i64,
...
pub(in crate::gateway) async fn stripe_source_pay(
    payment: &PaymentForCheckout,
    order: &PaymentOrder,
    source_type: &str,
...
pub(in crate::gateway) async fn stripe_checkout_pay(
    payment: &PaymentForCheckout,
    order: &PaymentOrder,
) -> Result<CheckoutResult, ApiError> {
    let config = payment_config(payment)?;
...
pub(in crate::gateway) async fn stripe_all_pay(
    payment: &PaymentForCheckout,
    order: &PaymentOrder,
) -> Result<CheckoutResult, ApiError> {
    let config = payment_config(payment)?;
    ...
    if method == "cards" {
        return stripe_all_cards_pay(payment.id, &config, order).await;
    }
    let currency = stripe_currency(&config)?;
    let exchange = exchange_cny_to(&currency).await?;
    let gateway_amount = stripe_payment_amount(order.total_amount, exchange, &currency)?;
    ...
    let payment_method_id = payment_method
        ...
        .and_then(Value::as_str)
        .ok_or_else(|| ApiError::legacy("Payment gateway request failed"))?;
    let user_email = order.user_email.clone().unwrap_or_default();
    let mut params = BTreeMap::from([
        ("amount".to_string(), gateway_amount.to_string()),
        ("currency".to_string(), currency.clone()),
...
async fn stripe_all_cards_pay(
    ...
    if let Some(email) = order.user_email.clone() {
...
pub(in crate::gateway) async fn stripe_source_notify(
    payment: &PaymentForCheckout,
...
pub(in crate::gateway) fn stripe_payment_intent_notify(
    payment: &PaymentForCheckout,
    input: &PaymentNotifyInput,
) -> Result<PaymentNotifyOutcome, ApiError> {
    let config = payment_config(payment)?;
...
pub(in crate::gateway) fn stripe_checkout_notify(
    payment: &PaymentForCheckout,
    input: &PaymentNotifyInput,
) -> Result<PaymentNotifyOutcome, ApiError> {
    let config = payment_config(payment)?;
...
pub(in crate::gateway) fn stripe_all_notify(
    payment: &PaymentForCheckout,
    input: &PaymentNotifyInput,
) -> Result<PaymentNotifyOutcome, ApiError> {
    let config = payment_config(payment)?;
...
pub(in crate::gateway) fn exchange_rate_cache_decision(
...
async fn fetch_exchange_rate(
    ...
    let value: Value = v2board_http_adapters::bounded_json(
        response,
...
pub(in crate::gateway) fn stripe_payment_amount(
    cny_cents: i32,
    exchange: Decimal,
    currency: &str,
) -> Result<i64, ApiError> {
...
pub(in crate::gateway) async fn stripe_cancel_intent(
    config: &serde_json::Map<String, Value>,
    intent_id: &str,
...
async fn stripe_response(response: reqwest::Response) -> Result<Value, ApiError> {
    ...
    let text = v2board_http_adapters::bounded_text(
        response,
        v2board_http_adapters::MAX_EXTERNAL_RESPONSE_BYTES,
...
pub(in crate::gateway) const STRIPE_WEBHOOK_TOLERANCE_SECS: i64 = 300;
...
pub(in crate::gateway) fn value_path_str(value: &Value, path: &[&str]) -> Option<String> {
