...
use super::{
    CheckoutResult, PaymentForCheckout, PaymentNotifyInput, PaymentNotifyOutcome, PaymentOrder,
    StripePaymentIntentResult, VerifiedPaymentNotify,
};
...
pub(super) async fn btcpay_notify(
    ...
    let response = payment_http_client("BTCPay")
        ...
        .error_for_status()
        .map_err(|_| ApiError::legacy("Payment gateway request failed"))?;
    let invoice: Value = v2board_http_adapters::bounded_json(
        response,
        v2board_http_adapters::MAX_EXTERNAL_RESPONSE_BYTES,
        "Payment gateway request failed",
    )
...
pub(super) async fn wechat_pay_native_pay(
    ...
    let response = payment_http_client("WechatPayNative")
        ...
        .await
        .map_err(|_| ApiError::legacy("Payment gateway request failed"))?;
    let response = v2board_http_adapters::bounded_text(
        response,
        v2board_http_adapters::MAX_EXTERNAL_RESPONSE_BYTES,
        "Payment gateway request failed",
    )
...
pub(super) async fn alipay_f2f_pay(
    ...
    let response: Value = v2board_http_adapters::bounded_json(
        upstream,
        v2board_http_adapters::MAX_EXTERNAL_RESPONSE_BYTES,
        "从支付宝请求失败",
    )
