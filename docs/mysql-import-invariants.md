...
3. 把 dump 导入一次性 staging MySQL，仅供 converter 读取；
...
它不需要永久安装在新服务器，也不需要运行在旧服务器。它可以是迁移机/新机上的 one-off
container、一次性 VM，或受控私网内的其他临时主机，并在成功或失败后连同临时数据一起删除。
新版长期运行不包含 MySQL 服务。
...
- staging MySQL 是新建、隔离、一次性的临时 engine/database，只允许导入器访问；
...
| `v2_plan` | `plan` | 全部保留 |
| `v2_payment` | `payment` | 排除 Stripe 配置 |
...
旧礼品卡使用记录转换为新的 `giftcard_redemption`。旧表没有兑换时间，因此这类派生行固定使用
...
因此 PostgreSQL 的 `log`、`mail_log`、`stat_user`、`stat_server`、`server_route`、上述丢弃的
协议 target（如 `server_shadowsocks`、`server_vmess`）和 `server_credential` 在导入结果中都必须
为空。这里的 `v2_*` 只指旧 MySQL source；不得把它重新用作 native target 名，也不得增加 rename
migration、alias 或兼容 view。
