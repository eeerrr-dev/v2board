...
4. 将固定保留的数据确定性转换并参数化写入全新专用的 PostgreSQL 18 cluster；
...
Rust converter 通过 MySQL driver 读取 typed row，逐字段验证和转换，再通过 PostgreSQL driver
使用参数化批量写入，显式处理 dialect、类型、自增、sequence、`NULL`、JSON 和约束差异。
...
`converted_snapshot_sha256` 绑定从 source snapshot 实际读出并转换的最终保留内容（含 deferred
inviter 关系）、逐表行数和单独表示的整表丢弃 present/absent 决策。discard-only 表不绑定 schema、
