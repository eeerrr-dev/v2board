...
状态：**F0 架构与 legacy schema v4 已冻结；legacy converter/executor 已接线，生产 fault gate 继续 fail closed**

本文定义三条且仅三条 lifecycle 路径：

- [`fresh_install`](examples/fresh-install.v3.example.json)：全新安装；
- [`legacy_reference_migration`](examples/legacy-migration.v4.example.json)：从唯一旧版迁移；
...
v3 保持严格只读兼容；legacy one-shot execution 只由严格 `schema_version: 4` 建模，v2 被明确拒绝。
...
cache、Stripe inventory 为零、临时订阅 URL 在切换时失效、节点策略为 `one_shot_offline_cutover`、旧
theme/custom rules 显式处置。当前实现的生产支持面进一步要求旧库节点 inventory 为空。
具体字段、Redis 语义和命令见[旧版迁移清单 v4](legacy-migration-manifest.md)。
...
或远程进程控制器。v4 不手填 `execution.nodes`；loader 固定注入空 inventory 与
`not_required_no_nodes`，重复填写会按 unknown field 拒绝。inspect 以
repeatable-read 读取旧 MySQL 八张节点表，任一记录都固定返回 `external_node_coordinator_unavailable`。
bulk copy 后 production executor 在 native authority 前后分别证明 PostgreSQL 八张节点表与
`v2_server_credential` 都为空，并要求同一 empty-set proof hash。
...
legacy `v2_stat` 和受 retention 保护的 `v2_stat_user/v2_stat_server` 必须在 PostgreSQL 逐值保真，且
...
`runtime.show_subscribe_method`，且 legacy v4 一律只允许 0 或 1；值 2 会被拒绝，避免复用算法导致旧
...
旧库存在任何节点时，本次迁移不会启动：inspect 明确报告
`external_node_coordinator_unavailable`。未来若把具体节点软件纳入支持范围，必须使用新 schema 同时交付
并审计 node-side 进程控制、credential、幂等报告和真实启动证明，不能扩写当前 empty-only 路径。
...
输出语义为 F0。改变未来 subscribe host/path/mode 属于 F1；schema-v4 one-shot 唯一例外是操作者明确
...
存在性/type/checksum inventory。目标 runtime 在 v4 spec 中完整手填，未知 key、缺 key、placeholder、
未处置 artifact 或无 provenance 都阻断。手填不是跳过自动现场检查的理由。

单一人工 v4 manifest 派生两个 `configuration_source=file_only` 文档：API 固定
...
- legacy 永久账本固定证明 `source_retired=true`、`mysql_reachable=false`、
  ...
  completion proof，也不得让人工清理失败改变已经提交的迁移结论；
...
| 能力 | 当前状态 | 生产结论 |
| --- | --- | --- |
| lifecycle JSON v3/v4 loader | v3 保留只读兼容；legacy v4 只手填外部事实与 custom-rules 选择，loader 注入固定 reference/七策略/空节点及内部路径，完整 hydrated 事实进入 HMAC scope | 只证明 spec 静态有效；v3 无 apply accessor，v4 也不自动解除实现 blocker |
...
| runtime config materialization | v4 可分别语义验证并原子安装 API/worker 两个最小 file-only map，校验 owner/0700/0600 | 只在总 fault gate 解除后由 legacy executor 执行 |
...
| legacy release/journal/backup/converter | one-shot 前向链路和 48 场景确定性恢复矩阵已实现；MySQL 8.0/8.4 的 27 表非空 fixture 已真实复制到 PostgreSQL 18，并完成 ClickHouse 26.3 空 epoch binding/readback | 必须完成全 checkpoint 真实裸机故障矩阵和最终安全审计后才可启用；不宣称 rolling rollback |

## 15. 代码与文档入口

- 数据库架构：[`docs/postgresql-clickhouse-invariants.md`](postgresql-clickhouse-invariants.md)
- legacy v4 指南：[`docs/legacy-migration-manifest.md`](legacy-migration-manifest.md)
- lifecycle 示例：[`fresh v3`](examples/fresh-install.v3.example.json)、
  [`legacy v4`](examples/legacy-migration.v4.example.json)、[`native v3`](examples/native-upgrade.v3.example.json)
- lifecycle grammar：[`backend/rust/crates/lifecycle/src/cli.rs`](../backend/rust/crates/lifecycle/src/cli.rs)
- v3/v4 spec/preflight：[`backend/rust/crates/provision`](../backend/rust/crates/provision)
