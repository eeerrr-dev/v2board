...
单一人工 v4 manifest 派生两个 `configuration_source=file_only`、`configuration_scope=boot_only` 启动文档：API 固定
...
| runtime config authority | v4 可分别语义验证并原子安装 API/worker 两个最小 boot-only file map；lifecycle 在服务启动前直接 seed 加密不可变 revision 并精确续跑，API/Worker 只加载同一 active revision、写角色独立 ack | role file 安装与 authority seed 只在总 fault gate 解除后执行；后台动态保存不写 role file；native upgrade executor 仍未实现 |
