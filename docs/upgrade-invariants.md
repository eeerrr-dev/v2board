...
`next_action=resolve_blockers`，`authorize` 会在显示人工确认提示前被拒绝，且 `apply/resume` 会在任何写入前
被拒绝。`v2board-api migrate` 只是已确认 native
...
`StripeCredit`、`StripeWepay`、`WechatPayNative`。本次 legacy 选择 `assert_none`；数据库检查将
`v2_payment.payment` 大小写不敏感地以 `stripe` 开头的每一行都计入 blocker，不因 `enable=0` 或历史未使用而
放行；unfinished Stripe order 或 provider-side 未处置 object 同样阻断。迁移器不会自动删除或禁用这些配置。
...
旧文件清单。数据库、Redis、release archive 与现场版本等已有 typed adapter 的事实仍自动检查；未知 key、缺 key、
placeholder 或 target provenance 缺失继续阻断。
...
single commit → API/worker 统一启动。v4 只接受空 source node inventory；v5 允许冻结并明确放弃非空 source
node inventory，但要求切换前停用旧远端进程、target 节点保持为空并在迁移后人工重建。两者都不存在
reporter 激活协议、bridge 或渐进放量。未来 native 破坏性升级若
