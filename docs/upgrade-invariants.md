...
状态：**F0 架构与 lifecycle schema v3 已冻结；converter/apply 尚未完成，生产继续 fail closed**

本文定义三条且仅三条 lifecycle 路径：

- [`fresh_install`](examples/fresh-install.v3.example.json)：全新安装；
- [`legacy_reference_migration`](examples/legacy-migration.v3.example.json)：从唯一旧版迁移；
...
`7e77de9f4873b317157490529f7be7d6f8a62421`。旧 MySQL、Percona 或 MariaDB 只可能是
`legacy_reference_migration` 的只读 source，绝不是新版 runtime target，也不存在 MySQL/MariaDB 与
PostgreSQL 可选双后端。

本文是目标规范，不代表 apply 已经可用。当前 CLI 只实现：

```text
v2board-lifecycle validate --manifest <path>
v2board-lifecycle inspect --manifest <path>
v2board-lifecycle plan --manifest <path>
```

它只接受严格 `schema_version: 3`；v2 被明确拒绝。当前 `inspect/plan` 固定输出
`converter_available=false`、`apply_available=false`、`verdict=blocked`、
`next_action=resolve_blockers`，仓库没有 lifecycle `apply`。`v2board-api migrate` 只是已确认 native
...
cache、Stripe inventory 为零、临时订阅 token 为零、节点一次性离线切换、旧 theme/custom rules 显式处置。
具体字段、Redis 语义和命令见[旧版迁移清单 v3](legacy-migration-manifest.md)。

固定顺序：

1. 旧系统仍服务时运行在线只读 `inspect`；报告通过后，操作者只决定一次是否启动不可逆 one-shot apply。
2. future apply 接受精确 `operation_id + inspect report_sha256` 后 fence 旧 API writer、Horizon/worker、
   scheduler 和 node reporters；停止临时订阅链接签发。
3. 以一次性离线 drain/importer 排空并对账 source Redis traffic、queue、failed work、paid-pending order
   和 reset barrier；不得建设 CDC、双写或长期 bridge。
4. 建立 source SQL、runtime artifact 和必要外部事件的一致 backup/PITR 起点，并完成隔离 restore drill。
5. apply 在 fenced 状态内部运行最终只读 recheck，重查 source/target identity、数据和 blocker；变化时
   在首次 mutation 前自动中止，不插入第二次人工等待。
6. final recheck 通过后先写 pending journal，再在同一维护操作内创建空 PostgreSQL/ClickHouse targets、bulk
   转换、逐值验证、物化 file-only config，并在全部节点保持离线时完成批量验证，最后只切流一次。

拒绝启动 apply 时不得进入维护或创建 target。旧 writer 与 native writer 永不双跑；成功提交后立即撤销 source 凭据、
网络隔离并永久停服 MySQL/MariaDB 与旧 Redis，只留带 checksum 的加密冷归档。pinned reference 与旧库
都永不作为生产回滚 runtime。
...
redemption time 时必须标记 unknown/synthetic provenance，不能伪造时间。
...
`requires_normalization`，不作为数据 violation；未来 converter 必须把它们写成 JSON number。零、负数、
...
节点身份是 `(node_type,node_id)`；group membership 必须是非空、引用真实 group 的 JSON array。node
协议字段、route、rate、port、parent 和 credential epoch 保留。节点 ID/type 变化会改变派生 token，普通
升级不得改变。

ticket/message ID、owner/author/status/reply status/time，knowledge/notice language/category/body/tag/sort，
mail/application logs 与 durable mail outbox 均按 retention 保留。`ticket_message.user_id=0` 是合法 admin
author，不可当 orphan。
...
事务。长期故障必须受 outbox 容量/磁盘水位/安全背压约束；该门禁尚未实现，因此仍阻断 production apply。

legacy `v2_stat` 和受 retention 保护的 `v2_stat_user/v2_stat_server` 必须保值。旧聚合只能作为明确
`legacy_aggregate` 迁移，不能伪造成不存在的 raw event。ClickHouse 任意投影都不得反写或决定订单、
支付、quota、subscription eligibility 与迁移成功。
...
旧 SQL `v2_user.token` 是永久订阅凭据，必须原值转换。Redis `otp_/otpn_/totp_` 是临时映射/cache；
mode 2 签发时甚至可能没有 key，所以必须同时有停止签发时间和完整过期窗口 proof。
...
节点固定 `one_shot_offline_cutover`：停止旧 API/worker 和全部 reporter，结清流量，为所有节点生成
scoped token 并写入稳定 idempotency key；native runtime 不包含全局 token fallback。逐节点检查只能
作为离线批量验收，任一节点未通过时所有
reporter 都继续停机。全部通过后统一启用新版，旧/新 reporter 永不并行。
...
订阅 token/uuid、完整 URL、mode 0/1/2 verifier、UA/flag、headers、General/Base64、Clash、Stash、
sing-box、Surge、Surfboard、Loon、Quantumult X、V2RayTun、SIP008、Shadowrocket、SagerNet 等实际
输出语义为 F0。改变未来 subscribe host/path/mode 属于 F1；已签发 URL 在 reset-security 或消费者
drain 前仍是 F0，旧 path 应直接服务内容而非只依赖 redirect。
...
存在性/type/checksum inventory。目标 runtime 在 v3 spec 中完整手填，未知 key、缺 key、placeholder、
未处置 artifact 或无 provenance 都阻断。手填不是跳过自动现场检查的理由。

单一人工 v3 manifest 派生两个 `configuration_source=file_only` 文档：API 固定
...
未来 apply 必须分别用 `0600` temp、fsync 和 atomic rename 写入；当前 CLI 只验证 map，不写文件。
...
- ClickHouse bootstrap、schema、仅具 raw INSERT + 批次核对 SELECT 的 relay writer、select-only
...
`APP_KEY`、database/Redis/ClickHouse/SMTP/reCAPTCHA/Telegram/payment/server credentials、secure path、
app/subscribe URL、trusted proxy/CORS、runtime/rules/frontend path 都属于 F1。轮换需要 old/new bridge 或
maintenance、消费者清单、外部重注册、fingerprint、回滚和审计。报告/日志/diff/argv 只能使用 secret
manager version 或独立 audit key HMAC，不能输出明文或低熵裸 hash。
...
single commit → API/worker/reporter 统一启动；这里不存在 bridge 或渐进放量。未来 native 破坏性升级若
...
- legacy Redis traffic/queue/failed work/subscription window drain 与 SQL/stat 对账；
...
- legacy 完成报告固定证明 `source_retired=true`、`mysql_reachable=false`、
  `source_credentials_revoked=true`、`legacy_tool_removed=true`、`legacy_runtime_compat=false`，并记录
  `cold_archive_reference + sha256`；PostgreSQL 是唯一事务 authority，ClickHouse 是分析投影；
...
`logout_all` 的旧 Laravel session 与 fence 后经分类确认可丢的 ephemeral cache。
...
| 能力 | 当前状态 | 生产结论 |
| --- | --- | --- |
| lifecycle JSON v3 loader | 已实现 strict kind-tagged shape、duplicate/unknown/missing key、regular/non-symlink/size/permission、完整 file-only runtime、TLS/secret/target/decision 校验及 v3 HMAC binding | 只证明 spec 静态有效，不授权写入 |
| fresh `inspect/plan` | 已有 target PostgreSQL/ClickHouse/Redis 只读 inventory | bootstrap、journal、config promote、backup/recovery/apply 未实现，固定 blocked |
| legacy `inspect/plan` | 已有 bounded source SQL/Redis/data 与 target inventory | converter、完整 source/config/topology/Stripe/Redis ownership proof、journal/cutover/apply 未实现，固定 blocked |
| native `inspect/plan` | v3 impact/epoch/confirmation metadata 已建模 | installation/build/schema machine binding、dry-run、impact estimator、journal/rollback/apply 未实现，固定 blocked |
| PostgreSQL native baseline/runtime | 新 lineage 和 runtime 已迁移到 PostgreSQL | 只能服务已确认 native database；不等于 lifecycle apply |
| typed PostgreSQL outbox/ClickHouse projection | schema、relay 和基础 integration proof 已建立 | 仍须随完整 production/capacity/restore gate 一起验收 |
| runtime config materialization | v3 可形成并分别语义验证 API/worker 两个最小 file-only map | CLI 不做 `0600` 写入、独立 secret mount 或原子 promote，仍是 apply blocker |
| lifecycle `apply` | 不存在 | 不支持生产 fresh install、legacy migration 或 destructive upgrade 自动执行 |
| release compatibility、journal、backup/restore、converter | 未完整实现 | 不支持 rolling/contract/drop/cutover 声明 |

## 15. 代码与文档入口

- 数据库架构：[`docs/postgresql-clickhouse-invariants.md`](postgresql-clickhouse-invariants.md)
- legacy v3 指南：[`docs/legacy-migration-manifest.md`](legacy-migration-manifest.md)
- 三个 v3 示例：[`fresh`](examples/fresh-install.v3.example.json)、
  [`legacy`](examples/legacy-migration.v3.example.json)、[`native`](examples/native-upgrade.v3.example.json)
- lifecycle grammar：[`backend/rust/crates/api/src/cli.rs`](../backend/rust/crates/api/src/cli.rs)
- v3 spec/preflight：[`backend/rust/crates/provision`](../backend/rust/crates/provision)
