...
手填一个严格 JSON 文件比猜测和转换旧配置更容易复查。当前有旧节点的站点使用 v5；v4 只保留其既有
“source 节点必须为空、全部旧统计表保留”语义。两者都以单个文件表达**目标意图**和**裸机执行意图**：
...
- 空 target Redis、公共行为配置和明确的迁移决策；API/worker 两个最终 boot runtime 路径由 loader 固定派生，
  不要求操作者重复填写。
...
要进入未来 one-shot apply，清单必须是 `schema_version: 4` 或 `schema_version: 5`，且 kind 必须是
`legacy_reference_migration`。当前非空节点来源必须使用 v5。旧 v3 仍可严格 `validate/inspect`，用于复查
已有文件，但它没有 `execution` accessor，永远不能授权 apply；不能只改版本号改变既定处置策略。
v2 已退役并被明确拒绝。

v4/v5 都不接受手填 `attestations`。writer 已停、队列已排空、restore 已通过和 source 已退役都是运行时才能
观测的事实，必须在动作成功后写入 hash-chain journal；预先写 `true` 不构成证据。执行型 inspect 输出也不再
显示 v3 的 `operator_attestations_complete` 状态，以免把“不适用”误读成未完成或已完成。
同样，固定 reference commit、公共固定策略和 target 空节点 execution 不再手填；工具在严格解析后注入，
并把原始 JSON 与完整 hydrated 事实一起纳入各 schema 独立的 manifest HMAC。旧冗余字段会按
`deny_unknown_fields` 拒绝。v5 要求操作者在 `decisions` 中明确写出节点与流量明细的破坏性处置，不能默认为放弃。

## 固定选择

`legacy_reference_migration` 不提供自由组合。公共策略由 loader 自动注入；v5 的 JSON `decisions` 必须显式
填写 `nodes`、`legacy_traffic_details`、`legacy_operational_logs` 和 `legacy_custom_rules`：
...
| 节点策略（v4） | `one_shot_offline_cutover` | 冻结兼容路径；source inventory 必须为空 |
| 节点策略（v5） | `discard_and_manual_rebuild` | 审计但不复制旧节点/路由；切换前人工停用旧远端节点，迁移后从空 target 重新建立 |
| 历史流量明细（v5） | `discard` | 不复制 `v2_stat_user`、`v2_stat_server`；保留 `v2_stat`、用户额度及累计 `u/d` |
| 瞬态历史日志（v5） | `discard` | 不复制 `v2_log`、`v2_mail_log`；完整 source rows 仍进入指纹与加密归档，copy receipt 证明旧行未进入 target |
| 旧 theme | `discard_confirmed` | 明确接受旧视觉/脚本资产不进入新版 |
| custom rules | `none` 或 `discard_confirmed` | 不盲拷旧模板；存在未处置文件时阻断 |

这里的 Stripe 零状态比“没有启用中的 Stripe”更严格：只要 `v2_payment.payment` 大小写不敏感地以
`stripe` 开头，该配置行就会阻断，不论 `enable` 是 0 还是 1，也不论它是否只是历史配置。迁移器不会
替操作者删除或禁用该行；unfinished Stripe order 与 provider-side object 还必须在迁移工具之外逐项处置。

同时固定：

- `runtime.configuration_source=file_only`；
- 工具派生 API/worker config 固定路径、`C.UTF-8`、target 必须为空、principal 必须不存在、ClickHouse
  standalone 及最小权限声明；这些不是 JSON 输入；
...
为避免 mode 2 在相同时间桶复用算法导致旧临时 URL 继续有效，legacy v4/v5 的目标
...
这两个 prefix 和下文的 Horizon prefix 在 v4/v5 都必须填写旧实例中的真实非空值；空字符串不能代表“没有
prefix”，否则 source drain 会扫描错误的物理 keyspace。只填一个 URL 可能产生危险的假零结果。当前
legacy 执行路径内部固定只支持 Redis cache driver，JSON 不再重复填写 `legacy_cache_driver`；真实来源若使用
...
- `execution.nodes` 不是 v4/v5 JSON 字段，手填会被当作 unknown field 拒绝。v4 内部注入
  `activation_transport.kind=not_required_no_nodes`；v5 注入
  `activation_transport.kind=discard_and_manual_rebuild`；两者 target `inventory=[]`。当前代码不存在远端节点激活协议。
- 在线 inspect 会在 repeatable-read 只读快照中精确读取旧 MySQL 的
  ...
  v4 任一记录都会返回稳定 blocker `external_node_coordinator_unavailable`；v5 则把完整集合、数量和摘要绑定为
  `discard_and_manual_rebuild` 证据，不要求面板伪装成远端协调器。
- v5 converter 不复制八张节点表、`v2_server_route`、`v2_stat_user`、`v2_stat_server`、`v2_log`、
  `v2_mail_log` 或派生 `v2_server_credential`。每张 source 表仍进入完整 27 表 fingerprint；保留表绑定
  PostgreSQL canonical 值，被放弃表则在独立 v5 fingerprint domain 中直接绑定
  完整 typed source row，不执行已经废弃的节点/流量/运行日志转换。最终报告记录行数、最大 ID 与
  `source_typed_rows_sha256`：它是按 ID 排序、保留 `SourceValue` 类型编码，并绑定独立 domain 与 source
  表名的原始行摘要；copy verification 与 native authority 提交前，13 张 discard target 都必须为零且
  identity sequence 固定为未调用状态。
- production executor 在 native authority 前后继续复核 v5 的八张节点表、route、credential、
  `v2_stat_user` 和 `v2_stat_server` 全为空。`v2_log`、`v2_mail_log` 的旧行未复制由持久 PostgreSQL receipt
  证明；authority 后 native API/worker 可以写入新的原生日志，因此此时不再要求两张日志表为空。
- `v2_stat` 继续逐值保留；`v2_payment` 的 ID、UUID、driver、验签 `config`、callback binding 与 `enable`
  全部原样迁移，迁移器绝不默认禁用旧支付方式。`v2_user` 的身份、套餐、余额、token、quota 以及
  `last_login_at`/`last_login_ip` 等现有字段也不做列级清洗或重置。
- 旧 V2bX/XrayR 等远端进程不由迁移器控制。操作者必须在切流前停用/屏蔽旧节点进程，迁移完成后使用新的
  `server_token` 和幂等策略手工重建；没有渐进激活、旧凭据 fallback 或新旧节点并行期。
...
`control_applied=true` receipt 的 SHA 预先塞进 manifest，会把承诺伪装成事实，v4/v5 会因 unknown field 或
不支持的 external-managed source 直接拒绝。

schema v5 还会在 journal 进入 `PostgresValueVerified` / `ClickhouseProjected` 前，分别写入
`postgres-verification-report.json` 和 `clickhouse-projection-report.json`。这两份 owner-only `0600`
回执保存 journal/永久账本所命名的**完整 JSON preimage**，并独立 HMAC 绑定 operation、installation、
manifest、inspect、source fingerprint/schema、converter strategy/registry 和输入 journal anchor。重试只接受
同一条 verified journal history 上的 clean anchor；PostgreSQL preimage 必须字节完全相同，ClickHouse 在
lost-ACK 后会重新独立验证当前投影，但仍返回首次已 fsync 回执的 hash。native authority 前以及 completion
时都会闭合验证 journal、回执和 authority 三方。加密 archive 与整个 operation 的 journal/receipts 是永久
审计材料；后续人工清理只删除一次性 binary 和 source credential，不得删除这些回执。

### 可执行备份模式

v4/v5 只接受 `mysql_logical_dump_and_isolated_restore`，不再让操作者选择 client family。执行器同时读取
...
source fence 后，执行器先做完整 27 表 fingerprint（v4 使用冻结 canonical serialization；v5 对保留表使用
canonical 值、对包括两张旧日志表在内的放弃表使用 raw typed row），并把已经 HMAC/Journal 验证、包含所有冻结流量
...
按批次日聚合表在切换前后必须全为空。v4 对三张旧统计表逐值验证；v5 只复制并逐值验证 `v2_stat`，同时
验证 `v2_stat_user`、`v2_stat_server` 的 source discard 摘要和 target 零行证明；`v2_log`、`v2_mail_log`
也由 PostgreSQL copy report 单独证明 source 摘要与 target 为空。两种策略都不会把旧聚合
伪造成历史 ClickHouse 事件。relay 对 raw 与日聚合分别使用稳定 token 并
...
1. `validate` 离线校验 v4/v5 文件、全部必填 key/type、URL、secret 独立性、派生路径契约和固定决策。
...
4. `apply` 先写入 fsync-durable pending journal，再在同一次调用中 durable fence 旧 API/scheduler，
   ...
   验证隔离恢复。v4 preflight 要求旧库节点 inventory 为空；v5 则冻结将被放弃的节点集合，并要求旧远端
   节点进程已在本次停机窗口中由操作者停用。
...
带 SHA-256 的永久 archive；operation journal 与签名 receipts 同样永久保留。
...
写入前失败。`authorize` 会重新执行在线只读检查并要求稳定 review binding 与操作者复查值一致，但当前在
显示确认提示、读取 operation ID 或写 authorization 前即因 capability 关闭而失败；未来启用后才会把当前完整
report 作为授权审计快照。确认前后都不会 fence 或修改 datastore。typed converter、durable
...
- 旧 MySQL 八张节点表的完整集合与 node-set digest；v4 非空时返回稳定 blocker，v5 报告明确丢弃数量；
