...
停止旧写入
  ...
  → typed converter 经同机房私网逐表 COPY FROM STDIN 到全新 PostgreSQL 18 cluster
  → COPY 后统一创建业务唯一约束、二级索引和外键、reset sequence、ANALYZE、逐表单次整表 canonical 校验
...
MySQL driver 对每张表只执行一条按主键排序的 streaming `SELECT`；内存有明确上限：当前 decoded row、
byte-bounded COPY send buffer，以及 Stripe 订单固定规则所需且最多 4096 项的 payment-id 分类索引。
数据逐字段校验和转换后，每张 target 表各自只有一条 PostgreSQL `COPY FROM STDIN`；礼品卡源流固定
同时生成基础表和兑换关系表。该缓冲只负责背压，不是 PostgreSQL batch；
没有固定 1000 行或其他批量 `INSERT`，也不在任何一台机器生成中间 COPY/CSV 文件。全部表 COPY
完成后才统一创建业务唯一约束、二级索引和外键，reset sequence、执行 `ANALYZE`，然后按主键顺序对每张
保留 target 表做且只做一次整表 canonical 校验。SQL dialect、类型、自增主键、`NULL`、布尔值、
JSON、金额精度、sequence 和约束差异都在这个唯一转换边界内处理。

## 1. 准备

准备以下资源：

- 与固定旧版 schema 匹配的 Oracle MySQL 8.0/8.3/8.4；
- 一个完整 MySQL dump；
...
- 旧生产机和新 PostgreSQL target 位于同一机房，并通过认证 TLS 的私网直接互通；
...
这条出站数据面固定走同机房私网和认证 TLS，不经公网，也不先落地中间传输文件。
...
旧机到 PostgreSQL endpoint 必须是同机房私网路径；跨公网、跨机房或先导出 COPY/CSV 再搬运都
不属于支持流程。
...
旧用户的 Telegram 绑定来自 MySQL `v2_user.telegram_id`，会原值进入 `users.telegram_id`；Bot token、
启用状态和群组配置仍须在 `runtime` 中显式填写。切换后必须在新版后台重新执行“设置 Webhook”，让
Telegram 使用新版 URL 和 secret-token 请求头。继续使用同一个 Bot 时用户无需重新绑定；若更换 Bot，
虽然 `telegram_id` 仍在，新 Bot 仍可能要求用户先与它交互后才能继续接收消息。
...
2. 在任何 target 写入前完成 source 正整数 primary key 与标量关系校验，并验证所有 target 全新为空；
   转换字段在逐行读取时校验，转换后才能判定的关系在 COPY 后最终验证；
3. 运行 PostgreSQL baseline 的 table phase，暂不创建业务唯一约束、二级索引和外键；
...
7. 按主键顺序对每张保留 target 表做且只做一次整表 canonical scan，与第 4 步累计的 expectation
   对比，并校验行数、键、标量关系与转换后的数组/派生关系、金额、流量和 token；
8. 证明 Stripe 行级规则和所有固定丢弃结果；
9. 保持 ClickHouse 旧事件为空，确认新 Redis 为空并持久化、重载、实测角色 ACL；
10. 生成并用生产 typed parser 验证 API/worker 配置，证明 bootstrap Redis secret 未进入输出；
11. 所有检查通过后才允许启动新版。

第 4 步的 MySQL 缓冲只是 bounded producer/consumer flow control；它不会产生 PostgreSQL 的 1000 行
`INSERT` batch。导入数据从 typed converter 直接进入 driver 的 `COPY FROM STDIN` stream，不生成
`.csv`、COPY payload 或其他中间行文件。第 7 步是每表唯一的一次 target 整表 canonical scan，
不存在逐 chunk/batch 对账、批量 `INSERT` fallback、可选传输模式或第二套导入方案。
...
整表丢弃 present/absent 决策。每张保留 target 表的 canonical 结果来自 COPY 后、按主键顺序进行的
唯一一次整表扫描，并与转换时累计的 expectation 对比；这些证据不能互相冒充。
...
- 用户 ID、password hash、永久 token、Telegram 绑定 `telegram_id`、余额、MySQL 已落盘的
  `u/d/transfer_enable`。
...
- 每张保留表完成 COPY，业务唯一约束、二级索引和外键已统一创建，sequence 已 reset，`ANALYZE` 已执行，且
  PK-ordered 单次整表 canonical 校验通过；
