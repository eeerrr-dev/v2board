...
2. legacy v4 只接受 `inventory=[] + not_required_no_nodes`；inspect 在 repeatable-read 中读取旧 MySQL 八张
   节点表，任一记录都会在进入维护窗口前以 `external_node_coordinator_unavailable` 阻断；bulk copy 后还要
   在 native authority 前后证明 PostgreSQL 对应八张表与 `v2_server_credential` 始终为空；
...
6. 旧 `v2_stat`、`v2_stat_server`、`v2_stat_user` 逐值迁移到 PostgreSQL，不能展开或估算成 raw traffic event；
...
- strict manifest/report schema v3/v4，以及 fresh/legacy/native-upgrade 三条只读检查流程；
...
到 PostgreSQL 18 的逐值复制、持久断点重建与幂等重跑，并完成 ClickHouse 26.3 lineage、installation binding、
...
此外，当前可执行 legacy 路径要求旧节点 inventory 为空。节点程序属于外部 V2bX/XrayR 等软件，本仓库
没有 node-side agent 或远程进程控制，故非空 inventory 固定以
`external_node_coordinator_unavailable` 阻断。未来非空支持必须使用新 schema 和完整 node-side 实现。
