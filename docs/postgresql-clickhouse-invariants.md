...
- MySQL importer 在停写后的旧生产机运行，通过专用 `SELECT`-only 账号和 server-enforced
  ...
  MySQL 5.7、Percona、MariaDB 和兼容代理全部拒绝。source patch version 不改变 PostgreSQL target。
...
4. MySQL driver 读取 typed row，converter 显式验证和转换，PostgreSQL driver 参数化写入固定保留行；
   MySQL dump SQL 不会交给 PostgreSQL 执行；
