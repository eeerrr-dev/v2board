...
`validate`、`inspect` 和 `execute`；`execute` 从已加载 dump 的旧机 staging MySQL 读取，
向不存在的 PostgreSQL/ClickHouse target 写入固定结果，验证新 Redis 为空，持久化角色 ACL，并生成
受限权限的配置输出包。
...
3. 在停写后的旧生产机把已校验绝对路径的 dump 导入隔离的一次性 staging MySQL 8 engine，且不运行旧 trigger、
   routine 或 event；staging 使用独立 data directory/volume、端口、凭据并仅本机监听，旧机容量
   不足时才使用一次性迁移 VM；
4. 在全新的 PostgreSQL 18 target 上运行当前 baseline，并转换固定保留行；
5. ClickHouse 从空 native event history 开始；新 Redis 8.8 使用专用 `/0`、整个 instance 从空状态
   开始，并由一次性 bootstrap user 建立、保存、重载及实测 API/worker ACL；
6. 从 `target` 和 `runtime` 在旧机不存在的 `config_output_directory` 生成
...
旧 MySQL 不是 target，也不被 converter 修改。staging 只是处理 dump 的临时输入。导入失败时删除
staging、不完整的新 PostgreSQL/ClickHouse/Redis target 和配置输出目录，修正问题，再从 dump
...
- `execute` 的 staging MySQL vendor/schema gate、PostgreSQL baseline/角色/行转换、ClickHouse 空历史
  bootstrap、空 Redis 验证、Redis ACL `SAVE`/`LOAD`/角色正负探针、配置输出与导入报告。

真实生产执行时仍必须用现场 dump、连接、网络策略和最终 Linux/systemd 环境完成第 7 节验收；
