...
- 全部 158 个唯一内部 API operation 由 Rust 注册表投影为 OpenAPI、TypeScript 与
  ...
  通配。64 个 JSON request body 与 95 个 JSON success representation 全部引用具名
  字段级 DTO，禁止 `JsonValue` 根类型；普通结构对象拒绝未知字段，只在规范明确允许
  的动态 map/extension island 保持开放。采用单一成功状态的生成 endpoint 会在 Zod
  解析响应体前校验该状态。wire DTO、页面模型和 form draft 仍是三个明确边界，不共用
  同一类型。
