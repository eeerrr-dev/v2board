...
describe('toast', () => {
  beforeEach(() => {
    ...
    render(createElement(Toaster, { theme: 'light' }));
  });
  ...
  it('keeps Sonner as the renderer instead of restoring the retired custom store or HTML sink', () => {
    const packageSource = join(process.cwd(), 'src/components/toaster.tsx');
    const workspaceSource = join(process.cwd(), 'packages/ui/src/components/toaster.tsx');
    const source = readFileSync(
      existsSync(packageSource) ? packageSource : workspaceSource,
      'utf8',
    );
    expect(source).not.toMatch(/\buseSyncExternalStore\b/);
