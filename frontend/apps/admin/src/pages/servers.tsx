...
function NodesTab() {
  ...
  const groupName = (ids: admin.ServerNode['group_id']) =>
    ids
      .map((id) => groups.data?.find((g) => g.id === Number(id))?.name ?? String(id))
      .join(', ');
  ...
  const columns: TableProps<admin.ServerNode>['columns'] = [
    { title: t('admin.server.node_id'), dataIndex: 'id', width: 70 },
    {
      ...
      dataIndex: 'show',
      width: 80,
      render: (v: admin.ServerNode['show'], row) => (
        <Switch
          size="small"
          checked={parseInt(String(v), 10) === 1}
          onChange={async (next) => {
            try {
    ...
    { title: t('admin.server.rate'), dataIndex: 'rate' },
    {
      ...
      dataIndex: 'group_id',
      render: (v: admin.ServerNode['group_id']) => groupName(v),
    },
    {
  ...
  return (
    <div className="space-y-4">
      ...
      <NodeEditDrawer
        ...
        id={editing?.id}
        record={nodes.data?.find(
          (node) => node.id === editing?.id && node.type === editing?.type,
        )}
        nodes={nodes.data ?? []}
        groups={groups.data ?? []}
        routes={routes.data ?? []}
