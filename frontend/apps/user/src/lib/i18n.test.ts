...
describe('i18n resources', () => {
  ...
  it('keeps the established zh-CN display copy in the static resource', () => {
    ...
    expect(i18n.t($ => $.dashboard.reset_in_days, { reset_day: 5 })).toBe('已用流量将在 5 日后重置');
    ...
    expect(i18n.t($ => $.knowledge.last_update, { date: '2026/06/04' })).toBe('最后更新: 2026/06/04');
  ...
  it('renders the complete statically owned en-US resource', () => {
    ...
    expect(i18n.t($ => $.node.status_tip)).toBe('Access Point online status in the last 5 minutes');
    expect(i18n.t($ => $.dashboard.used_traffic, { used: '1 GB', total: '10 GB' })).toBe(
  ...
  it('registers complete localized resources for every supported locale', () => {
    ...
    expect(i18n.t($ => $.knowledge.last_update, { date: '2026/06/04' })).toBe('最後更新: 2026/06/04');
    ...
    expect(i18n.t($ => $.order.processing)).toBe('Hệ thống đang xử lý đơn hàng, vui lòng đợi 1-3p.');
    ...
    expect(i18n.t($ => $.invite.current_commission_balance)).toBe('Số dư hoa hồng giới thiệu hiện tại');
    expect(i18n.t($ => $.invite.transfer_placeholder)).toBe('Hãy nhậo số tiền muốn chuyển đến số dư');
