...
// to reproduce that for CSS *animation* consumers (the slide-up select dropdown).
// CSS *transition* consumers (the drawers) omit it and activate on the next
// frame, which is the correct mechanism for a class-toggled transition.
export function useTransitionStatus(
  ...
  // enter -> entering/entered: hold the base-only class before adding "-active".
  useEffect(() => {
  ...
  // rc-animate removes enter/enter-active after the CSS animation finishes.
  useEffect(() => {
  ...
  // leave -> leaving: same base-only hold before adding "-active".
  useEffect(() => {
